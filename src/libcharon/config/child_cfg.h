...
struct child_cfg_t {
	 ...
	 * Returned list and its traffic selectors must be destroyed after use.
	 *
	 * Produces a log message and triggers an alert if narrowing occurred.
	 *
	 * @param local			TRUE for TS on local side, FALSE for remote
 ...
 * Refer to select_traffic_selectors() for details (the difference is that this
 * can work with external traffic selector lists).
 *
