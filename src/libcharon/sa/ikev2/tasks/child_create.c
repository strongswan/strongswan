...
static void handle_notify(private_child_create_t *this, notify_payload_t *notify)
{
	...
	switch (notify->get_notify_type(notify))
	{
		...
		case USE_AGGFRAG:
			...
			this->iptfs_flags = *(uint8_t*)data.ptr;
			break;
...
/**
 * Prepare proposed traffic selectors as initiator when not rekeying/recreating.
 */
	...
	private_child_create_t *this, child_sa_t *old)
{
	...
	this->my_ts = traffic_selector_list_create_from_enumerator(
										old->create_ts_enumerator(old, TRUE));
	this->other_ts = traffic_selector_list_create_from_enumerator(
