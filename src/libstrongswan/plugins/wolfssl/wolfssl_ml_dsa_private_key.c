	...
	void *params, chunk_t data, chunk_t *signature)
{
	...
	*signature = chunk_alloc(sig_len);
								 ...
								 lib->ns))
	{
		...
		ret = wc_dilithium_sign_ctx_msg_with_seed(ctx, strlen(ctx),data.ptr,
					 data.len, signature->ptr, &sig_len, &this->key, seed);
	}
	else
	{
		WC_RNG rng;
		...
		ret = wc_dilithium_sign_ctx_msg(ctx, strlen(ctx), data.ptr, data.len,
