 ...
 */
static private_private_key_t *create_empty(key_type_t type, uint8_t level)
{
	private_private_key_t *this;
...
private_key_t *wolfssl_ml_dsa_private_key_gen(key_type_t type, va_list args)
{
	...
	if (wc_dilithium_make_key_from_seed(&this->key, seed.ptr) ||
...
private_key_t *wolfssl_ml_dsa_private_key_load(key_type_t type, va_list args)
{
	...
	this->keyseed = chunk_clone(priv);
	this->pubkey = chunk_alloc(pubkey_len);
	len = pubkey_len;

	/* derive private and public key from seed */
	if (wc_dilithium_make_key_from_seed(&this->key, priv.ptr) ||
		wc_dilithium_export_public(&this->key, this->pubkey.ptr, &len))
	{
		DBG1(DBG_LIB, "deriving %N from seed failed", key_type_names, type);
		destroy(this );
