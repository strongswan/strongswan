...
bool wolfssl_ml_dsa_enabled(key_type_t key, uint8_t *level, size_t *pubkey_size);
...
static private_private_key_t *create_instance(key_type_t type, uint8_t level,
											  chunk_t keyseed, size_t pubkey_len)
{
	private_private_key_t *this;
	u_int len;
	...
	if (wc_dilithium_make_key_from_seed(&this->key, keyseed.ptr) ||
...
private_key_t *wolfssl_ml_dsa_private_key_gen(key_type_t type, va_list args)
{
	...
	if (!wolfssl_ml_dsa_enabled(type, &level, &pubkey_len))
	{
		return NULL;
	}
	...
	this = create_instance(type, level, seed, pubkey_len);
	if (!this)
	{
		return NULL;
	}
...
private_key_t *wolfssl_ml_dsa_private_key_load(key_type_t type, va_list args)
{
	...
	if (priv.len == 0 || !wolfssl_ml_dsa_enabled(type, &level, &pubkey_len))
	{
		return NULL;
	}
	...
	this = create_instance(type, level, chunk_clone(priv), pubkey_len);
	if (!this)
	{
		return NULL;
	}
