							  ...
							  ml_dsa_poly_t *s2, ml_dsa_poly_t *t0)
{
	...
	for (j = 0; j < l; j++)
	{
		...
		{
			if (!bitpacker->read_bits(bitpacker, &value, d))
			{
				goto end;
			}
						...
						ml_dsa_poly_t *y)
{
	...
	for (j = 0; j < l; j++)
	{
		...
		if (!this->H->set_seed(this->H, rho) ||
			!this->H->get_bytes(this->H, 32*(1 + gamma1_exp), v.ptr))
		{
			return FALSE;
		}
								...
								ml_dsa_poly_t *z, ml_dsa_poly_t *h)
{
	...
	/* encode vector of polynomials with binary coefficients h */
	...
	void *params, chunk_t data, chunk_t *signature)
{
	...
	chunk_t rho, K , tr, ctx, seed, mu, rnd, rho_pp, w1_enc, c_tilde;
	int32_t bound1 = (1 << this->params->gamma1_exp) - this->params->beta;
	int32_t bound2 = gamma2 - this->params->beta;
	u_int kappa = 0;
	rng_t *rng;
	...
	if 	(lib->settings->get_bool(lib->settings, "%s.ml_dsa.deterministic", FALSE,
								 lib->ns))
	{
	...
	while (TRUE)
	{
		...
		/* deoompose elements of vector w into high and low bits */
...
static bool generate_keypair(private_private_key_t *this, chunk_t keyseed)
{
	...
	if (!encode_public_key(this, rho, t1) ||
...
private_key_t *ml_dsa_private_key_load(key_type_t type, va_list args)
{
	...
	if (priv.len == 0 || !ml_dsa_matching_type(type))
	{
		return NULL;
	}
