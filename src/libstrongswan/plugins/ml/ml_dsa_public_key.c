							 ...
							 chunk_t c_tilde, ml_dsa_poly_t *z, ml_dsa_poly_t *h)
{
	...
	/* decode vector of polynomials with binary coefficients h */
	...
	for (i = 0; i < k; i++)
	{
		...
		while (index < sig.ptr[omega + i])
		{
			...
			{
				DBG1(DBG_LIB, "error: signature with non-encreasing hint positions");
				return FALSE;
			}
...
/**
 * Reads an optional context string from strongswan.conf
 */
chunk_t ml_dsa_get_context(void)
{
	char *context_str;
	chunk_t context, context_hex, context_bin;
	size_t len = 0;

	context = chunk_alloc(2 + 255);
	context.ptr[0] = 0;
	context.ptr[1] = 0;
	context.len = 2;

	context_str = lib->settings->get_str(lib->settings,
					"%s.ml_dsa.context", NULL, lib->ns);
	if (context_str)
	{
		len = strlen(context_str);

		/* context in hex format? */
		if (len > 2 && context_str[0] == '0' && context_str[1] == 'x')
		{
			context_hex = chunk_create(context_str + 2, len - 2);

			if (context_hex.len  > 2 * 255)
			{
				DBG1(DBG_LIB, "context string has more than 255 bytes");
			}
			else
			{
				context_bin = chunk_from_hex(context_hex, context.ptr + 2);
				context.ptr[1] = (uint8_t)context_bin.len;
				context.len += context_bin.len;
			}
		}
		else
		{
			...
			{
				memcpy(context.ptr + 2, context_str, len);
				context.ptr[1] = (uint8_t)len;
				context.len += len;
			}
		}
	}
...
/**
 * samples a poliynomial c with coefficients from {-1, 0, 1}
	...
	void *params, chunk_t data, chunk_t signature)
{
	const u_int k = this->params->k;
	const u_int l = this->params->l;
	int32_t bound = (1 << this->params->gamma1_exp) - this->params->beta;
	ml_dsa_poly_t a[k*l], t1[k], z[l], h[k], w1[k], c;
	chunk_t rho, c_tilde, c_tilde2, w1_enc, tr, ctx, seed, mu;
	...
	ctx = ml_dsa_get_context();
	seed = chunk_cat("cmc", tr, ctx, data);
	...
	if (!chunk_equals(c_tilde2, c_tilde))
	{
...
/**
 * Check matching ML-DSA key type
 */
bool ml_dsa_matching_type(key_type_t type)
{
	switch (type)
	{
		case KEY_ML_DSA_44:
...
public_key_t *ml_dsa_public_key_load(key_type_t type, va_list args)
{
	...
	if (!ml_dsa_matching_type(type) || blob.len == 0)
	{
		return NULL;
	}
