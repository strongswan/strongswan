...
START_TEST(test_ml_dsa_load)
{
	...
	/* loading a private key with an incorrect key size must fail */
	short_key = chunk_skip(tests[_i].seed, 1);
	key = lib->creds->create(lib->creds, CRED_PRIVATE_KEY, tests[_i].type,
							 BUILD_PRIV_ASN1_DER, short_key, BUILD_END);
	ck_assert(key == NULL);

	/* load private key */
	key = lib->creds->create(lib->creds, CRED_PRIVATE_KEY, tests[_i].type,
							 BUILD_PRIV_ASN1_DER, tests[_i].seed, BUILD_END);
	ck_assert(key != NULL);
	...
	/* loading a public key with an incorrect key size must fail */
