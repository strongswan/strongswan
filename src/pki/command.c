...
static bool process_common_opts(bool init)
{
	...
	bool success = TRUE, found_command = FALSE;
	...
	while (TRUE)
	{
		...
		{
			case '+':
				/* only process --options until we found a known command during
				 * initialization, otherwise we process them twice */
				if (!found_command &&
					!options->from(options, optarg, &argc, &argv, optind))
				{
					success = FALSE;
					break;
				}
...
int command_init(int c, char *v[])
{
	...
	build_opts();
	/* handle common options until we might stop due to an unknown argument */
