							 ...
							 configuration_attribute_type_t type)
{
	...
	{
		g_variant_builder_add (&builder, "u", g_variant_new_uint32 (*(uint32_t*)chunk.ptr));
	}
...
static gboolean disconnect(NMVpnServicePlugin *plugin, GError **err)
{
	...
	{
		/* enqueue the actual disconnection, because we may be
		 * called in response to a child_state_change() and the
		 * SA enumeration would possibly deadlock. */
