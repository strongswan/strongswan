...
int LLVMFuzzerInitialize(int *argc, char ***argv)
{
	...
	library_init(NULL, "fuzz_vici_message");
	libcharon_init();
	return 0;
}
...
int LLVMFuzzerTestOneInput(const uint8_t *buf, size_t len)
{
	...
	/* Fuzz VICI message creation and parsing */
	data = chunk_clone(chunk_create((u_char*)buf, len));
	msg = vici_message_create_from_data(data, TRUE);

	/* Fuzz VICI message methods */
	if (msg)
	{
		...
		int count = 0;
		while (count++ < 10000 && enumerator->enumerate(enumerator, &type, &name, &value))
		{
		}
